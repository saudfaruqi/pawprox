          ...
          <li key={req.id} className="p-3 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-100 dark:border-yellow-800/30">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                {req.senderPic ? (
                  <img
                    src={req.senderPic.startsWith('http') ? req.senderPic : `http://localhost:5001/${req.senderPic}`}
                    alt={req.senderName}
            ...
            <div className="flex items-center justify-between">
              ...
              >
                <div className="relative">
                  {friend.profilePic ? (
                    <img 
                      src={friend.profilePic.startsWith('http') ? friend.profilePic : `http://localhost:5001/${friend.profilePic}`} 
                      alt={friend.username} 
...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    const url = `http://localhost:5001/api/users?search=${encodeURIComponent(debouncedSearch)}`;
    ...
    fetch(url, {
      ...
      .then(data => {
        ...
        Promise.all(
          filtered.map(user =>
            fetch(`http://localhost:5001/api/friends/check/${user.id}`, {
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('token')}`
              }
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    const url = `http://localhost:5001/api/friends/accepted`;
  ...
  useEffect(() => {
    if (socket && selectedUser && friendStatuses[selectedUser.id]) {
      ...
      fetch(`http://localhost:5001/api/chat/conversation?receiver_id=${selectedUser.id}`, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  const sendFriendRequest = (userId) => {
    fetch('http://localhost:5001/api/friends/send', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleAcceptRequest = (requestId, senderId) => {
    fetch(`http://localhost:5001/api/friends/accept/${requestId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      .then(data => {
        ...
        fetch(`http://localhost:5001/api/users/${senderId}`, {
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
  ...
  const handleDeclineRequest = (requestId) => {
    fetch(`http://localhost:5001/api/friends/decline/${requestId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleRemoveFriend = (friendId) => {
    openModal({
      ...
      onConfirm: () => {
         fetch(`http://localhost:5001/api/friends/remove/${friendId}`, {
           method: 'DELETE',
           headers: {
             'Content-Type': 'application/json',
             'Authorization': `Bearer ${localStorage.getItem('token')}`
